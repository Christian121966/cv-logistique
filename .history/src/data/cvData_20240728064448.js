const cvData = {
  formation: [
    {
      date: "Mars 2023",
      title: "Intégrateur Web chez Openclassroom",
      // details: "HTML/CSS - JavaScript - React - Redux - MongoDB - Git - GitHub - Responsive design"
    },
    {
      title: "Capacité de transport de marchandises",
    },
    {
      title: "DUT Gestion, Comptabilité"
    }
  ],
  competences: {
    developpementWeb: [
      { title: true, text: "HTML5, CSS3" },
      { title: false, text: "Utilisation des balises sémantiques avec HTML pour chaque élément d'un site web de réservation de vacances" },
      { title: false, text: "Apprendre à utiliser des balises sémantiques pour améliorer l'accessibilité et le référencement de votre site web." },
      { title: true, text: "JavaScript (ES6+)" },
      { title: false, text: "Réalisation d'un carrousel du site d'une imprimerie" },
      { title: false, text: "Créer un carrousel d'images interactif en utilisant JavaScript." },
      { title: false, text: "Utilisation de JavaScript pour créer de manière dynamique le site web d'un architecte" },
      { title: false, text: "Utiliser JavaScript pour générer du contenu dynamique et interactif sur un site web en communiquant avec une API." },
      { title: true, text: "React.js" },
      { title: true, text: "Responsive design" },
      { title: false, text: "Implémentation d'un site d'une plateforme de restauration en mobile-first avec SASS" },
      { title: false, text: "Couvrir les techniques de conception mobile first en utilisant SASS pour garantir un site optimisé pour les appareils mobiles avant de l'adapter aux écrans plus grands. Découvrir comment utiliser les fonctionnalités avancées de SASS comme les variables, les mixins, et les fonctions pour créer des styles modulaires et réutilisables." },
      { title: true, text: "Optimisation SEO" }
    ],
    logistique: [
      { title: true, text: "Capacité de transport de marchandises" },
      { title: false, text: "Gestion d'une société de transport de marchandises" },
      { title: false, text: "Gestion de personnel" },
      { title: false, text: "Gestion des stocks" },
      { title: true, text: "Communication" },
      { title: false, text: "Gestion des contacts" },
      { title: false, text: "Gestion du personnel" }
    ]
  },
  experiences: [
    {
      date: "Déc. 2021 - Juin 2022",
      title: "First Stop Sartrouville - Herblay",
      details: [
        { highlight: "Livraison de pneus en Ile de France, gestion des commandes et des stocks" },
      ]
    },
    {
      date: "2007 - 2021", 
      title: "Gérant de société de transport - Paris",
      details: [
        { highlight: "Livraison de colis en partenariat avec Colisweb" },
        { highlight: "Livraison avec SVP" }, 
        { highlight: "Livraison avec CG Courses" }
      ]
    },
    {
      date: "1999 - 2007",
      title: "Cariste gestionnaire de stocks - Rougier Panneaux - Gennevilliers",
      details: [
        { highlight: "Chargement et déchargement de colis de panneaux de bois" },
        { highlight: "Gestion du stock" }, 
        { highlight: "Préparation des commandes" }
      ]
    }
  ]
};

export default cvData;
